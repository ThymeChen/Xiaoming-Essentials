...
public class RemoteInteractor extends SimpleInteractors<EssentialsPlugin> {
    ...
    Pattern miraiCode = Pattern.compile("\\\\\\[mirai\\\\:(.*)\\\\:(.*)\\\\]");
    ...
    public void sendMessage(PrivateXiaoMingUser user, @FilterParameter("消息") String message) {
        groupContact.ifPresentOrElse(contact -> {
            if (miraiCode.matcher(message).find()) {
                contact.sendMessage(MiraiCode.deserializeMiraiCode(message).contentToString());
            } else {
                contact.sendMessage(MiraiCode.deserializeMiraiCode(message).serializeToMiraiCode());
            }
